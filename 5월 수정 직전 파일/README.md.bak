...
- 표지 이미지(파일·URL·표지 검색 모달), 삽화(자동/수동) 삽입
...
### 🖼 표지 검색 모달
- 카카오페이지·리디북스·네이버시리즈·조아라·문피아 플랫폼 링크 제공
- 표지 파일 직접 드롭 또는 URL 붙여넣기 지원
...
```
index.html         — 단일 페이지 SPA 마크업 (1,221줄)
├── style.css      — CSS 변수 74개+, 키프레임 14개+ (1,232줄)
├── core.js        — StateManager, Toast, SettingsDB, EventBus, 공통 유틸 (530줄)
├── convert.js     — 변환 탭 로직: 파일 I/O, 인코딩, 이미지, 삽화 처리 (2,790줄)
├── parser.js      — 텍스트 파싱 엔진, 목차 렌더링, 가상 스크롤, 통계 (2,044줄)
├── ui-state.js    — 테마, CSS 미리보기, 탭 전환, 칩 빌더, regexFeed, 스와이프 (1,025줄)
├── settings.js    — CSS/폰트/프리셋/스킨, 스마트 패턴, 히스토리, Gemini API (1,317줄)
├── edit.js        — EPUB 편집 탭: OPF 파싱, 챕터 수정, 재생성 (1,603줄)
├── cover-search.js — 표지 검색 모달, 플랫폼 링크 스트립, 이미지 크롭 (497줄)
...
> **총 코드량: 13,121줄** | 구문 오류 0개 | 중복 ID 0개
...
```
novel2epub/
├── index.html       # 메인 HTML (SPA, 1,221줄)
├── style.css        # 테마·컴포넌트·애니메이션·스킨 (1,232줄)
├── core.js          # StateManager, Toast, SettingsDB, EventBus (530줄)
├── convert.js       # 변환 탭: 파일 I/O, 인코딩, 이미지, 삽화 (2,790줄)
├── parser.js        # 텍스트 파싱, 목차 추출, 가상 스크롤, 통계 (2,044줄)
├── ui-state.js      # 테마, CSS 미리보기, 탭/칩/피드/스와이프 (1,025줄)
├── settings.js      # CSS 설정, 프리셋, 스마트 패턴, 히스토리 (1,317줄)
├── edit.js          # EPUB 편집 탭: OPF 파싱·챕터 수정·재생성 (1,603줄)
├── cover-search.js  # 표지 검색 모달, 플랫폼 스트립, 이미지 크롭 (497줄)
├── epub-gen.js      # EPUB3 생성, Blob Worker, 이미지 병렬 처리 (389줄)
├── worker.js        # Web Worker: 인코딩 감지·디코딩·챕터 파싱 (354줄)
└── sw.js            # Service Worker: 오프라인 캐시, PWA (119줄)
```
